...
export async function generateMetadata({ params }: { params: Promise<{ slug?: string }> }): Promise<Metadata> {
  ...
  if (!username) {
    return {
      title: 'Links — JassSpace',
      description: 'View links and profile on JassSpace.',
    };
  }
  ...
  const title = `${displayName} — JassSpace`;
  const description = `${displayName}'s JassSpace page: links, socials, and profile.`;
  const keywords = `JassSpace, links, profile, ${username}`;
