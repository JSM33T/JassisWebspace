...
using JassSpace.Worker.Configuration;
...
public sealed class IpGeolocationService : IIpGeolocationService
{
    ...
    public async Task<IpGeolocationResult?> ResolveAsync(IPAddress? ipAddress, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Handle both ip-api.com and ipapi.co response formats
            var result = new IpGeolocationResult(
                Latitude: TryGetDouble(root, "lat") ?? TryGetDouble(root, "latitude"),
                Longitude: TryGetDouble(root, "lon") ?? TryGetDouble(root, "longitude"),
                City: TryGetString(root, "city"),
                Region: TryGetString(root, "regionName") ?? TryGetString(root, "region") ?? TryGetString(root, "region_name"),
                Country: TryGetString(root, "country") ?? TryGetString(root, "country_name"),
    ...
    {
        var baseUrl = _options.BaseUrl?.TrimEnd('/') ?? "http://ip-api.com";
        
        // ip-api.com format: http://ip-api.com/json/{ip}
        // ipapi.co format: https://ipapi.co/{ip}/json/
        if (baseUrl.Contains("ip-api.com"))
        {
            return $"{baseUrl}/json/{ipAddress}";
        }
        else
        {
