...
public sealed class AdminTierController(
    ...
    : BaseApiController
{
    ...
    {
        var tiers = await tierService.GetAdminTiersAsync(cancellationToken);
    ...
    public async Task<IActionResult> CreateBillingCycle( [FromBody] TierBillingCycleCreateRequest? request, CancellationToken cancellationToken = default)
    {
        if (request is null)
        ...
        try
        {
            var cycle = await tierService.CreateBillingCycleAsync(request, cancellationToken);
            return OkEnvelope(cycle);
        }
        catch (TierNotFoundException ex)
        {
            logger.LogWarning(ex, "Attempt to create billing cycle for missing tier {TierId}", request.TierId);
            return NotFoundProblem("Tier not found", ex.Message);
        }
        catch (TierServiceException ex)
        {
            logger.LogWarning(ex, "Billing cycle creation failed for tier {TierId}", request.TierId);
            return BadRequestProblem("Unable to create billing cycle", ex.Message);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unhandled error creating billing cycle for tier {TierId}", request.TierId);
            return Problem(
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status404NotFound)]
    public async Task<IActionResult> UpdateBillingCycle( int billingCycleId, [FromBody] TierBillingCycleUpdateRequest? request, CancellationToken cancellationToken = default)
    {
        if (request is null)
            return BadRequestProblem("Request body is required.");
        ...
        try
        {
            var cycle = await tierService.UpdateBillingCycleAsync(billingCycleId, request, cancellationToken);
            return OkEnvelope(cycle);
        }
        catch (TierServiceException ex)
        {
            logger.LogWarning(ex, "Billing cycle update failed for cycle {CycleId}", billingCycleId);
            return BadRequestProblem("Unable to update billing cycle", ex.Message);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unhandled error updating billing cycle {CycleId}", billingCycleId);
            return Problem(
                statusCode: StatusCodes.Status500InternalServerError,
                title: "Internal Server Error",
    ...
    public async Task<IActionResult> DeleteBillingCycle(int billingCycleId, CancellationToken cancellationToken = default)
    {
        try
        {
            await tierService.DeleteBillingCycleAsync(billingCycleId, cancellationToken);
            return NoContent();
        }
        catch (TierServiceException ex)
        {
            logger.LogWarning(ex, "Billing cycle delete failed for cycle {CycleId}", billingCycleId);
            return BadRequestProblem("Unable to delete billing cycle", ex.Message);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unhandled error deleting billing cycle {CycleId}", billingCycleId);
            return Problem(
                statusCode: StatusCodes.Status500InternalServerError,
                title: "Internal Server Error",
